...
public class GeetestController {
    ...
    @Value("geetest.captchaKey")
    private String captchaKey;


    @GetMapping(value = "/loginCheckCode")
    @ApiOperation("验证码:用户登录")
    public R userLogin(@RequestParam Map<String, String> getParams) {
        // 1.初始化极验参数信息
        ...
        if (jsonObject.getString("result").equals("success")) {
            return R.success(jsonObject.get("reason"));
        } else {
            return R.fail(jsonObject.get("reason"));
        }
